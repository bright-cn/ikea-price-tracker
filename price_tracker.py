"""
IKEA Price Tracker - Bright Data Web Scraper API
----------------------------------------------------
Track prices, availability, and product data from IKEA.

Docs  : https://docs.bright.cn/scraping-automation/web-scraper-api/overview
Info  : https://bright.cn/products/insights/price-tracker/ikea
"""
...
def track_prices(urls: list[str], **kwargs) -> list[dict]:
    """
    Collect price data for a list of IKEA product URLs.
...
def discover_by_keyword(keyword: str, limit: int = 50, **kwargs) -> list[dict]:
    """
    Discover IKEA products by keyword search.
...
def discover_by_category(category_url: str, limit: int = 100, **kwargs) -> list[dict]:
    """
    Discover all products from a IKEA category page.

    Example:
        results = discover_by_category("https://ikea.com/s?k=headphones", limit=200)
    """
...
if __name__ == "__main__":
    ...
    sample_urls = [
        "https://www.ikea.com/us/en/p/sample-product-12345678/",
        # Add more IKEA product URLs here
    ]
    results = track_prices(sample_urls)

    output_file = "ikea_prices.json"
    ...
    # cat_results = discover_by_category("https://ikea.com/category/sample", limit=50)
